...
[`encoding.md`](callpackage.md). This document records the design
...
- **Indexed class**: one class indexed by `Symbol` (`class Package name`).
  ...
  This is the direction taken in `encoding.md`.
...
(`'AtLeast ('V 3 0 0)`). These appear in `encoding.md` as `Deps`
constraints.
...
return derivations. Adopted directly in `encoding.md`.
...
in `encoding.md` as the associated constraint family.
